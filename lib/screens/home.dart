        ...
        () {
          return StaggeredGridView.countBuilder(
              ...
              itemBuilder: (context, index) {
                ...
                DateTime time = DateFormat.Hm().parse(task.startTime.toString());
                var myTime = DateFormat("HH:mm").format(time);

                if (task.repeat == 'Day') {
                  if (task.remind == 1) {
                    notifyHelper.scheduledNotification(
                      hour: int.parse(myTime.toString().split(":")[0])-24,
                      minute: int.parse(myTime.toString().split(":")[1]),
                      task: task,
                    );
                  } else if (task.remind == 2) {
                    notifyHelper.scheduledNotification(
                      hour: int.parse(myTime.toString().split(":")[0])-48,
                      minute: int.parse(myTime.toString().split(":")[1]),
                      task: task,
                    );
                  } else if (task.remind == 3) {
                    notifyHelper.scheduledNotification(
                      hour: int.parse(myTime.toString().split(":")[0])-72,
                      minute: int.parse(myTime.toString().split(":")[1]),
                      task: task,
                    );
                  } else {
                    notifyHelper.scheduledNotification(
                      hour: int.parse(myTime.toString().split(":")[0])-96,
                      minute: int.parse(myTime.toString().split(":")[1]),
                      task: task,
                    );
                  }
